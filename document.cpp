...
struct Document::Data {
    ...
    pdf_obj* dict(const char* key) const { return pdf_dict_gets(ctx, pdf_trailer(ctx, pdf()), key); }
    void loadInfoDict() {
        if (!info) { info = dict("Info"); }
    }
    bool load() {
...
Document::Document() : d(new Data) { fz_register_document_handlers(d->ctx); }
...
bool Document::load(const QString& fileName) {
    ...
    fz_try(d->ctx) { d->mdoc = fz_open_document_with_stream(d->ctx, "pdf", d->stream); }
    fz_catch(d->ctx) {
...
bool Document::isLocked() const { return d->locked; }
...
Page Document::page(int pageno) const { return Page(d->ctx, d->mdoc, pageno); }

QList<QByteArray> Document::infoKeys() const {
    QList<QByteArray> keys;
    if (!d->mdoc) { return keys; }

    d->loadInfoDict();

    if (!d->info) { return keys; }

    const int dictSize = pdf_dict_len(d->ctx, d->info);
    for (int i = 0; i < dictSize; ++i) {
        pdf_obj* obj = pdf_dict_get_key(d->ctx, d->info, i);
        if (pdf_is_name(d->ctx, obj)) { keys.append(QByteArray(pdf_to_name(d->ctx, obj))); }
    }
    return keys;
}

QString Document::infoKey(const QByteArray& key) const {
    if (!d->mdoc) { return QString(); }

    d->loadInfoDict();

    if (!d->info) { return QString(); }
...
fz_context* Document::ctx() const { return d->ctx; }

fz_document* Document::doc() const { return d->mdoc; }

float Document::pdfVersion() const {
    if (!d->mdoc) { return 0.0f; }
    char buf[64];
    if (fz_lookup_metadata(d->ctx, d->mdoc, FZ_META_FORMAT, buf, sizeof(buf)) != -1) {
        int major, minor;
        if (sscanf(buf, "PDF %d.%d", &major, &minor) == 2) { return float(major + minor / 10.0); }
    }
    return 0.0f;
}

Document::PageMode Document::pageMode() const { return d->pageMode; }

/******************************************************************************/

Outline::Outline(const fz_outline* out) {
    if (out->title) { m_title = QString::fromUtf8(out->title); }
    if (out->uri) { m_link = std::string(out->uri); }
}

Outline::~Outline() { qDeleteAll(m_children); }
